...
public class CirclePercentView extends View {
    ...
    public void setCurPercent(final float curPercent) {
        ...
        anim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                mCurPercent = curPercent;//四舍五入保留到小数点后两位
                invalidate();
            }
        });
        anim.start();
    ...
    public CirclePercentView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mArcWidth = ta.getDimensionPixelSize(R.styleable.CirclePercentView_arcWidth, DensityUtil.dip2px(context, 16));
        mPercentTextColor = ta.getColor(R.styleable.CirclePercentView_percentTextColor, 0xffffee00);
        mPercentTextSize = ta.getDimensionPixelSize(R.styleable.CirclePercentView_percentTextSize, DensityUtil.sp2px(context, 16));
        mRadius = ta.getDimensionPixelSize(R.styleable.CirclePercentView_radius, DensityUtil.dip2px(context, 100));
        ta.recycle();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawArc(mArcRectF, 270, 360 * mCurPercent, false, mArcPaint);

        String text = String.valueOf(mCurPercent);
